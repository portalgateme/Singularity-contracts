...
/**
 * @title CurveFSNRemoveLiquidityAssetManager.
 * @dev Contract to remove liquidity from Curve stable ng factory pools.
 */
contract CurveFSNRemoveLiquidityAssetManager is
    BaseAssetManager,
    CurveInputBuilder,
    CurveAssetManagerHelper
{
    ...
    function curveRemoveLiquidity(
        ...
        if (!_validateAssets(_getMetaFactoryCoins(_args.pool),_args.assetsOut)) 
        {
    ...
    function _removeLiquidity(
        ...
        if (count == 1) {
            ...
            if (coinNum <= 4) {
                uint256[] memory curveInputAmounts = new uint256[](coinNum);
                uint256[] memory outAmounts = new uint256[](coinNum);
