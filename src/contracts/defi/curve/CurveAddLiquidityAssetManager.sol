 ...
 * @dev Contract to add liquidity to Curve
 *      plain / lending /crypto pools.
 */
contract CurveAddLiquidityAssetManager is
    BaseAssetManager,
    CurveInputBuilder,
    CurveAssetManagerHelper
{
    ...
    function curveAddLiquidity(
        ...
        if (
            !_validateAssets(_getCoins(_args.pool), _args.assets) 
        ) {
            if (
                !_validateAssets(_getUnderlyingCoins(_args.pool), _args.assets)
            ) {
    ...
    function _addLiquidity(
        ...
        if (coinNum == 2) {
            ...
            //1100
            //mintAmount = _args.poolFlag & 12 == 12 ? 
            ...
            if (_args.poolFlag & 2 == 2){
                uint256 initAmount = IERC20(_args.lpToken).balanceOf(address(this));
                
                if (ethAmount > 0 && _args.poolFlag & 1 != 1){
                    ILegacyPools(_args.pool).add_liquidity {value: ethAmount} (
                        curveInputAmounts,
                        mintAmount
                    );
                } else {
                    ILegacyPools(_args.pool).add_liquidity(
                        curveInputAmounts,
                        mintAmount
                    );

                }
                mintAmount = IERC20(_args.lpToken).balanceOf(address(this)) - initAmount;
            //01    
            } else if (_args.poolFlag & 1 == 1) {
                ...
                if (ethAmount > 0) {
                    if (_args.poolFlag & 32 == 32) {
                        mintAmount = IPools(_args.pool).add_liquidity{
                            value: ethAmount
                        }(curveInputAmounts, mintAmount);
                    } else {
                        mintAmount = IPools(_args.pool).add_liquidity{
                            value: ethAmount
                        }(curveInputAmounts, mintAmount, _args.booleanFlag);
                    }
                } else {
                    if (_args.poolFlag & 32 == 32) {
                        mintAmount = IPools(_args.pool).add_liquidity(
